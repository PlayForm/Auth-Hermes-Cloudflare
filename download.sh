...
#
# Security properties:
#   - downloads ONLY the pinned release asset + its checksums (never curl | bash)
#   - checksum trust model (in order of preference):
#       1. the plugin tree's SHA256SUMS.txt (GENERATED BY Build.yml, pinned in
#          the tree) - trusted only when its BINARY_VERSION header matches the
#          version being installed and it has a block for this target;
#       2. the release's per-target SHA256SUMS-<target>.txt asset;
#       3. neither available -> loud WARNING and the sha256 check is SKIPPED
#          (older releases stay installable) - never a silent install.
#     A checksum that IS available but does NOT match is always a hard error.
#   - verifies the archive sha256 against the resolved sums BEFORE extraction
...
# --- resolve checksum source: in-tree SHA256SUMS.txt -> release asset -> warn ---
# The plugin tree carries SHA256SUMS.txt GENERATED BY Build.yml (consolidated
# from the per-target release assets; committed by the build workflow) - the
# pinned tree is the checksum source of truth. It is used only when its
# BINARY_VERSION header matches the version being installed; the target's
# block (`<target>:` header, then the `hash  auth-cloudflare-<target>.tar.gz`
# lines until the next `...:` header) is the sums input, same format as the
# per-target release assets. If neither source yields sums, print a loud
# WARNING and skip verification (older releases stay installable); a checksum
# that IS available but does NOT match is always a hard error.
SUMS_OK=0
...
if [[ -f "$IN_TREE_SUMS" ]]; then
	...
	# extract this target's block; trust it only if the pinned version matches
	if [[ -n "$IN_TREE_VERSION" && "$IN_TREE_VERSION" == "$BIN_VERSION" ]]; then
		awk -v want="${TARGET}:" '$1 == want { in_block=1; next } in_block && $1 != "" && $1 !~ /:$/ { print } in_block && $1 ~ /:$/ && $1 != want { exit }' "$IN_TREE_SUMS" 2>/dev/null >"$TMP_SUMS"
		if [[ -s "$TMP_SUMS" ]]; then
			SUMS_OK=1
			echo "OK - checksums from in-tree ${IN_TREE_SUMS} (BINARY_VERSION ${IN_TREE_VERSION})"
		fi
	fi
fi
if [[ "$SUMS_OK" -eq 0 ]]; then
	# fallback: the release's per-target sums asset (same format)
	if curl -fL --retry 3 --connect-timeout 10 --max-time 60 -o "$TMP_SUMS" "$SUMS_URL" 2>/dev/null; then
		SUMS_OK=1
		echo "OK - checksums from release asset ${SUMS_URL}"
	else
		echo "WARNING: no checksum source available - SKIPPING sha256 verification for this release" >&2
		echo "  looked for: in-tree ${IN_TREE_SUMS} (generated by Build.yml) and release asset ${SUMS_URL}" >&2
		echo "  this release will install WITHOUT checksum verification; the binary still self-validates" >&2
	fi
fi
...
if [[ "$SUMS_OK" -eq 1 ]]; then
	...
	if [[ ! "$expected" =~ ^[0-9a-f]{64}$ ]]; then
		echo "WARNING: resolved sums have no valid checksum entry for ${ASSET} - skipping sha256 check for this asset" >&2
	else
